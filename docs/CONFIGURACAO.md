...
1. Baixe o instalador:
   - **Release** (recomendado): página *Releases* do repositório → `RealOficial-Setup-1.0.0-x64.exe`;
   - ou *Actions → CI → artifacts → RealOficial-Setup-x64* (builds de desenvolvimento).
2. Execute o instalador. Como o executável ainda **não é assinado digitalmente**, o Windows
   SmartScreen pode exibir um aviso azul — clique em **“Mais informações” → “Executar assim mesmo”**.
3. Escolha a pasta (instalação por usuário, não exige administrador) e conclua.
4. Abra o **Real Oficial** pelo atalho. Na primeira abertura, a tela "Iniciando o motor…" aparece
   por alguns segundos enquanto o serviço local sobe.
...
4. Ao terminar, a galeria mostra os **cortes ordenados por score (0–100)**. Clique num corte para
   revisar: pré-visualização 9:16, ajuste de início/fim (−1s/+1s), estilo de legenda, kit de marca,
   detalhamento dos 18 parâmetros → **Aprovar** ou **Rejeitar**;
...
| Rosto errado em foco no 9:16 | Ajuste o trecho (o plano é recalculado) ou use um corte com modo blur |
| Legenda com termo errado | Edite no modal de revisão (correção por palavra) e re-renderize |

## 11. Atualizações

Novas versões chegam como novas tags/releases — baixe o novo `RealOficial-Setup-*.exe` e instale
por cima (dados e configurações são preservados; o desinstalador não apaga `engine-data`).
